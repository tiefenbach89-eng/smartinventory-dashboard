...
type RoleKey = 'admin' | 'manager' | 'operator' | 'employee';
...
export default function AccountsPage() {
  ...
  // Rollen-Metadaten (Labels + Beschreibungen, voll übersetzt)
  const roleOptions: { value: RoleKey; label: string; desc: string }[] = [
    ...
    {
      value: 'operator',
      label: tRoles('operator'),
      desc: tRoles('operator_desc')
    },
    {
      value: 'employee',
    ...
    <PageContainer>
      <div className='w-full space-y-6 px-4 py-6 sm:px-6 md:px-10 md:py-10'>
        ...
        <Tabs defaultValue='users' className='w-full'>
          {/* Tabs oben wieder kompakt, nicht über die ganze Breite */}
          <TabsList className='bg-card/25 border-border/10 flex h-auto w-fit flex-wrap items-center justify-center gap-2 rounded-3xl border px-2 py-2 text-sm shadow-[0_0_20px_rgba(255,255,255,0.05)] backdrop-blur-md sm:h-11 sm:w-fit sm:flex-nowrap sm:py-0'>
            <TabsTrigger
              value='users'
                                ...
                                <TableCell>
                                  <div className='flex items-center gap-2'>
                                    {isAdmin ? (
                                      ...
                                      >
                                        <SelectTrigger className='bg-background border-border/50 w-[170px]'>
                                          <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                                          {roleOptions.map((r) => (
                                    ...
                                    {/* Tooltip-Icon mit Rollenbeschreibung */}
                                    <TooltipProvider delayDuration={150}>
                                      <Tooltip>
                                        <TooltipTrigger className='border-border/60 text-muted-foreground hover:bg-muted/40 inline-flex h-5 w-5 items-center justify-center rounded-full border text-[10px]'>
                                          i
                                        </TooltipTrigger>
